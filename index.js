...
let currBlockedState = false
let hasOpenArduinoPort = false
...
client.on('RT_CAR_INFO', (
    ...
    function checkWheelsLocking(wheelAngularSpeeds, lockingThreshold) {
        let results = [];
        const sum = wheelAngularSpeeds.reduce((a, b) => a + b, 0);
        const avg = (sum / wheelAngularSpeeds.length) || 0;
        const averageAngularSpeed = avg
        for (let i = 0; i < wheelAngularSpeeds.length; i++) {
            let currentAngularSpeed = wheelAngularSpeeds[i];
            if (Math.abs(currentAngularSpeed - averageAngularSpeed) > lockingThreshold) {
                results.push(true);
            } else {
                results.push(false);
            }
    ...
    currBlockedState = hasBlockedWheel;
    port.write(Number(hasBlockedWheel)+'')
    console.log(Number(hasBlockedWheel)+'')
})
// Start listening
client.start();
...
// Вам потребуется библиотека SoftwareSerial.h. Создайте новый файл в Arduino IDE и скопируйте следующий код:
...
// mySerial.begin(9600); // Скорость порта
// }

// void loop() {
// if(mySerial.available()){
// digitalWrite(ledPin, mySerial.read() == '1' ? HIGH : LOW);
// }
// }
// В коде выше мы создали виртуальный пин на цифровом пине 2 и подключили его к SoftwareSerial. Мы также проверяем, есть ли какие-то данные на виртуальном пине, и если есть, то включаем или выключаем светодиод.
